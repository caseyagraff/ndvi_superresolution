...
class GanModelTrainer:
    def __init__(self, model, params, sample_dir=None):
        ...
        # Set device
        self.device = torch.device(params.general.device) if torch.cuda.is_available() else torch.device('cpu')

        # Setup optimizer
        self.discrim_optimizer = torch.optim.Adam(model.parameters(), self.params.learning_rate)
        self.gen_optimizer = torch.optim.Adam(model.parameters(), self.params.learning_rate)

        # Setup loss
        self.gan_loss = model.gan_loss
        self.perceptive_loss = loss_functions.select_perceptive_loss(self.params.perceptive_loss, self.params)
    ...
    def train(self, data, num_epochs, val_data=None):
        ...
        for epoch in tqdm.tqdm(range(num_epochs)):
            ...
            for batch_id, (x_low, x_high) in enumerate(train_data_loader):
                ...
                # Compute perceptive loss
                gen_loss += self.perceptive_loss(x_high_gen, x_high)
            ...
            if val_data:
                ...
                output_str += f', Val Loss: {val_loss}'
    ...
    def compute_validation_loss(self, val_data):
        ...
        with torch.no_grad():
            ...
            for x_low, x_high in val_data_loader:
                discrim_loss, gen_loss = self.loss(self.generator, self.discriminator, x_low, x_high)
        ...
        return discrim_loss_total, gen_loss_total
